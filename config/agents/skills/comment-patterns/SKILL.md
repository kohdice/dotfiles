...
- When logic is genuinely non-obvious and structure cannot absorb it (rename/extract considered first), write the why-comment — omission is the opposite failure. Constraints the type system cannot express (locking order, required call ordering, the unit of a bare integer) must be written at the site that depends on them. When the value carrying a constraint is extracted to a named constant or function, attach the rationale to that declaration, once; use sites reference the name — comment a use site only when it depends on the constraint in a way the declaration comment does not cover.
...
Acceptance exempts a comment from redundancy findings only — truth is orthogonal. A comment of any accepted class whose verifiable claim contradicts the code is a misleading comment; fix it by making the claim true while preserving the accepted-class content (the citation, unit, or safety rationale), never by deleting it. Acceptance is judged against the code as written: a comment is accepted when the current code cannot carry the information — whether a type or structure change could absorb it instead is a code-design question outside a comment review.
...
- **Reviewing**: verify before flagging — read the adjacent code and check whether every claim in the comment is derivable from it; when a claim references another symbol, verify against that symbol's declaration and usage too, not just the annotated code. A cross-reference is misleading only when the referenced symbol's actual behavior contradicts the claim — an informal but true reference is accepted; borderline claims are classified with the misleading entry's falsifiability test. A reference to anything external (issue, spec, measured number, platform quirk) is not derivable — keep it. If redundancy cannot be demonstrated after reading the code, do not flag.
