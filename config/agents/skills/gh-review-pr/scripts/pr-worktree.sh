...
setup() {
  ...
  parent="${TMPDIR:-/tmp}/gh-review-pr-worktrees"
  ...
  printf '%s\n' "$repo_root" >"$worktree/.gh-review-pr-worktree"
...
cleanup() {
  ...
  worktree="$1"
  marker="$worktree/.gh-review-pr-worktree"

  if [ ! -f "$marker" ]; then
    echo "Refusing to remove worktree without .gh-review-pr-worktree marker: $worktree" >&2
    exit 1
  fi
