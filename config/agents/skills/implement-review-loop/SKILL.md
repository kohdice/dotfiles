...
1. **Implement**: Before implementation, record the baseline commit hash (`git rev-parse HEAD`), initial status (`git status --porcelain=v1 -uall`), and starting content of files the task will edit, including files already dirty. Keep snapshots outside the review scope without staging or stashing. Run the implement skill's workflow end to end (plan resolution, baseline, batch execution — direct or delegated — with evidence, milestone full-suite runs). Commit interaction is this loop's responsibility, not the implement skill's: suppress its final commit suggestion — the loop is not done and the code is not yet reviewed. Its phase reporting applies unchanged: report each phase's `After this phase:` outcome and full-suite result and continue; pause only for a checkpoint the user asked for or a decision that blocks the next batch. Commits are suggested exactly once, in the final report (step 6), where the git-commit skill can split the working tree into one commit per phase. If the implement skill concludes the request has no testable behavior, this skill does not apply either; say so and stop. Track the loop's changed files and edits from implementation/fix evidence and comparisons with the starting state; status alone does not establish ownership, and an already-dirty path must not be omitted just because it was dirty initially. Update this scope after every fix round; capture a newly touched existing file before editing it. If HEAD differs from the recorded hash at exit, attribute the movement in the report (this loop never moves it; concurrent external activity can).

2. **Review**: Run the local-review skill's workflow over the loop's changes: the working diff when the tree was clean at loop start, otherwise the loop-owned edits relative to the recorded starting content. Give reviewers the owned file list and that comparison; a whole-file path scope alone is insufficient when a file also contains unrelated pre-existing edits. Those edits may provide context but must not produce blocking or advisory findings for this loop; enforce this during synthesis as well as dispatch. If the user named a base, use the branch diff instead. Orchestration bookkeeping (the plan file, anything under `.plans/`) is never part of the review scope, regardless of git tracking status. Lens selection and synthesis follow `review-lenses.md` unchanged. Execution differs from standalone local-review in one respect: the direct-review path (the parent reviewing a small scope itself) never applies here — every round dispatches fresh reviewer sub-agents, never a previous round's, because the ledger's recurrence check depends on reviewers who have not seen the earlier round.

3. **Gate and ledger**:
   - **Gate**: simplicity-only findings remain advisory even when their severity is `high`. A merged finding can block only when a non-simplicity lens independently justifies blocking severity. Among the remaining findings, blocking = severity `high` (plus `medium` only if the user asked for a stricter gate at the start — never tighten the gate mid-loop). Non-blocking findings, however trivial their fixes look, pass through to step 6 unmodified. If there are no blocking findings, go to step 6.
   - **Ledger update**: match every blocking finding against the ledger (format below), record every new one, and flag all matches to findings marked fixed in an earlier round. Finish recording the entire review before choosing an exit, so new findings are retained even when another finding recurs.
   - **Recurrence check**: if any fixed finding recurred, **stop the loop** and report every remaining blocking finding (step 6). Mark new findings from this review `unfixed — recurrence stop`. Mechanical repetition will not resolve what a targeted fix could not.
...
4. **Fix**: Increment the fix-round counter when starting this round, before its first fix pass; a round that later stops on failure still consumes one round, and recovery stays within that round. For each blocking finding, make **one** fix pass, sequentially (fixes share files and build on each other): a fix dispatch to a fresh sub-agent, or a direct fix in the parent when the implement skill's direct-execution rule applies (small, clear, one file, no catalog beyond the idiom baseline needed). Either way the pass adapts the contract in `implementer-dispatch.md`:
   ...
   - Mark findings fixed in the ledger only after their fix evidence and the end-of-round full suite pass. If recovery fails, leave this round's findings `unfixed — regression stop`, report the failure and changed files, and stop; attempting a fix is not verification that it landed.

5. **Loop**: Return to step 2 for a fresh review of the updated diff. The round was already counted in step 4.

6. **Report**: One final report containing: rounds used — fix rounds spent out of the cap, plus the number of review rounds run; blocking findings fixed (from the ledger); remaining blocking findings and the exit reason (cap, recurrence, or failed recovery); all advisory/lower-severity findings from the last review, untouched; the final suite result — the most recent parent-run full-suite result: the last fix round's end-of-round run, or the implement phase's plan-completion run when no fix round occurred (do not re-run the suite just for the report); and the review mode (delegated or inline), with implementation and fix execution reported separately (direct or delegated for each). Report language precedence: a standing user-level language directive (e.g. the user's CLAUDE.md) > the conversation language; code, identifiers, and paths stay in English (the same rule as local-review's synthesis). State the baseline hash outcome — `HEAD unchanged from baseline <hash>` in the normal case, or the attributed movement. Suggest committing via the git-commit skill (for a phased plan, one commit per phase). Never commit automatically.
...
When a finding is left unfixed at a cap exit, `unfixed — cap reached` replaces the whole `fixed: round M` segment. For a new finding left unfixed at a recurrence exit, use `unfixed — recurrence stop` instead. For findings left unresolved by a failed recovery, use `unfixed — regression stop`. Keep a recurring finding's earlier fix and recurrence history. One example of each cap/fixed state:
