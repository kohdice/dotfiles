---
name: implement
description: This skill should be used when the user asks to implement application code with testable behavior — "implement a feature", "〜を実装して", "execute the TDD plan", "プランを実装して" — or says bare "go" after a TDD plan was created in the same session. It drives TDD implementation from a plan in `.plans/` (created first via the tdd-plan skill when none exists). The parent owns the plan file, the milestone full-suite runs, and the final report; each batch runs directly in the parent when it is small and clear, or in an implementer sub-agent that follows the tdd skill and reports test evidence. Runtime-neutral (Claude Code, Codex). Do NOT use for plan creation only (tdd-plan skill), an explicit `/tdd` invocation (tdd skill), or a request to implement and iterate on review findings in one go (implement-review-loop skill).
---
