...
- Do NOT commit, push, stash, change branches, or alter refs or staged paths, modes, or content. Assigned working-tree edits and expected test artifacts are allowed; read-only Git inspection may refresh internal metadata without violating this constraint.
  ...
  "properties": {
    ...
    "suite": {
      ...
      "properties": {
        ...
        "tier": {
          "type": "string",
          "enum": ["fast", "fast+integration", "full"]
        },
...
- All other invariants hold unchanged: fast tier at every checkpoint, full suite at milestones, plan writes only between items, preserve Git history and staging, discovered tests appended not implemented.
