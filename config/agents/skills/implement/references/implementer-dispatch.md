...
| `performance-patterns`                                  | When the batch touches hot-path-shaped code: parsers, encoders, request handlers, per-item/per-frame loops. Also read its `references/<lang>.md` for the target language. |
