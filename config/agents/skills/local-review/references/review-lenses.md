...
| sql          | `sql-performance-reviewer`                                                              | `sql-performance`                                                                 | SQL statement cost and result correctness, gated by the resolved engine (PostgreSQL / MySQL / SQLite): query multiplication (N+1, correlated subqueries), non-SARGable predicates, over-fetching, sort/pagination cost, NULL traps in `NOT IN`, unbatched writes, index definitions serving hot statements     | High = a `[wrong-result]` statement, or a per-request / per-item query cost       |
...
- `sql`: when the change touches `.sql` files, migration files, or SQL embedded in code (string literals or query-builder calls containing `SELECT` / `INSERT` / `UPDATE` / `DELETE` / `WITH` / `CREATE INDEX`). One lens regardless of engine — the knowledge skill resolves the engine.
...
- Files outside C/Go/Rust/Zig get only `correctness`, except `.sql` and migration files, which also get `sql`. This rule wins over the per-scope lists above: the idiom and cross-language lenses apply only to files in those four languages, so when every in-scope file is outside them, dispatch `correctness` (plus `sql` for SQL files) and list the other lenses as skipped.
...
- Overlap rules when the named agents are dispatched: `correctness` must not comment on style or idiom (the idiom reviewers own official style), structure (architecture), complexity (simplicity), runtime cost (performance), or comment prose (comment). `comment` must not flag doc-comment format or mandated presence (the idiom lenses own official format) nor commented-out code blocks (simplicity owns dead code). `performance` must not judge the cost of SQL statements themselves (`sql` owns statement shape, query count, and index definitions); `sql` must not judge the application code around a query (performance owns it) nor schema design — keys, constraints, data types, normalization — which no lens owns yet and is reported as out of scope.
...
- **Units**: Go → one chunk per package; Rust → per crate (per top-level module for a large crate); Zig → per `build.zig` module or top-level directory; C → per directory or library boundary. Files outside the four languages form per-directory `correctness`-only chunks, reported in coverage like any other chunk; `.sql` and migration directories run `sql` as well.
- **Per-chunk lenses**: `idiom`, `performance`, `simplicity`, `comment`, `sql`, `correctness` run per chunk.
