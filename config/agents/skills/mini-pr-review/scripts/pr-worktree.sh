...
setup() {
  ...
  parent="${TMPDIR:-/tmp}/mini-pr-review-worktrees"
  ...
  printf '%s\n' "$repo_root" >"$worktree/.mini-pr-review-worktree"
...
cleanup() {
  ...
  worktree="$1"
  marker="$worktree/.mini-pr-review-worktree"

  if [ ! -f "$marker" ]; then
    echo "Refusing to remove worktree without .mini-pr-review-worktree marker: $worktree" >&2
    exit 1
  fi
