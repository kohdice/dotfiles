...
When the PR's diff touches C, Go, Rust, or Zig source files, additionally dispatch from this set. First detect the languages present (`gh pr diff --name-only` + extensions), then dispatch **one idiom reviewer per language present** and the four cross-language reviewers. State the target language(s) and the changed file list explicitly in each dispatch prompt. These named agents exist only in Claude Code; in other runtimes cover the same lenses via generic sub-agents or the inline fallback — in that case each lens's knowledge skill (below) supplies the checklist.
