...
Detailed material for SKILL.md workflow step 3 (fan out the review). Defines the review lenses, the named agents, the read-only sub-agent dispatch contract, the findings schema sub-agents must return, and how the parent synthesizes them into a single verdict.

## Runtime-aware agent selection

Each lens names its preferred agents. Use the matching named agent when it is loaded in the current runtime, whether it comes from a runtime-specific agent directory, a plugin, or another agent registry. When a preferred agent is not loaded, dispatch a generic read-only sub-agent with the lens prompt instead. If no sub-agent can be dispatched at all, run the selected lenses inline in the parent, single-pass, and label the result single-pass in the review summary.

- Runtime-specific directories and plugins are implementation details. This skill addresses reviewers by role name and falls back when a role is unavailable.
...
| Lens | Focus | What counts as a blocker | Preferred named agents |
| --- | --- | --- | --- |
...
| security | Injection, secret exposure, missing authz, unsafe input handling, data loss, migrations, compatibility | Any exploitable path, leaked secret, data loss risk, or breaking compatibility change | `pr-review-security-data-compatibility`, or a generic read-only agent |
...
When the PR's diff touches C, Go, Rust, or Zig source files, additionally select from this set by what the diff touches. First detect the languages present (`gh pr diff --name-only` + extensions), then dispatch **one idiom reviewer per language present**, and add the cross-language reviewers whose concern the diff raises: `architecture` when files were added or moved, import/include/use blocks changed, or a new package/module/crate appeared; `performance` when the diff touches hot code (per-item, per-request, per-frame paths, or loops over input-sized data); `simplicity` when the diff adds or reshapes an abstraction (interface/trait, generic, builder, wrapper, layer, option); `comment` when the diff adds or edits comments or doc comments. When unsure whether a condition holds, dispatch the reviewer; log every reviewer not dispatched with its reason. State the target language(s) and the changed file list explicitly in each dispatch prompt. Use the named agent when the current runtime has loaded it; otherwise cover the same lens via a generic sub-agent or the inline fallback, with the lens's knowledge skill supplying the checklist.

| Lens | Focus | What counts as a blocker | Named agent | Knowledge skill |
| --- | --- | --- | --- | --- |
...
| comment | Comment content value and accuracy: restating/journal/banner noise, echo doc comments missing the real contract, stale or misleading comments | A comment whose claim the code contradicts (actively misleads about behavior) | `comment-reviewer` | `comment-patterns` |
...
When these language-aware agents are dispatched, narrow the overlapping generic lenses to avoid duplicate findings: limit `design` to type/data design (structure belongs to `architecture`, complexity to `simplicity`), skip generic style commentary (the idiom reviewers own official style), and limit the generic `comments` lens to files outside C/Go/Rust/Zig — the `comment` lens owns comment accuracy and content value for those languages.
