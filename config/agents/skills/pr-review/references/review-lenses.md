...
When the PR's diff touches C, Go, Rust, or Zig source files, additionally dispatch from this set. First detect the languages present (`gh pr diff --name-only` + extensions), then dispatch **one idiom reviewer per language present** and the four cross-language reviewers. State the target language(s) and the changed file list explicitly in each dispatch prompt. Use the named agent when the current runtime has loaded it; otherwise cover the same lens via a generic sub-agent or the inline fallback, with the lens's knowledge skill supplying the checklist.
