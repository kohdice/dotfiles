...
6. **Verify and record**: Read the batch's evidence. It is complete when every item reports its before-check, its Verify command as run, an output summary, and a result, and every summary matches the plan's expected outcome verbatim. When a summary does not match verbatim, evidence is missing, or fields contradict each other, re-run that item's Verify step in the parent before deciding.
