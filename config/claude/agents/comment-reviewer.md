...
description: Use this agent when you need to review source-code comments for content value and accuracy — comments that restate what the adjacent code already says, journal/process narration that belongs in version control, banner and closing-brace markers, doc comments that echo the signature while leaving the real contract unstated, stale or misleading comments contradicting the code, dead TODOs, and missing why-comments on genuinely non-obvious logic. Invoke after writing or modifying code, before committing, or when auditing an existing codebase for comment noise. Covers C, Go, Rust, and Zig. Do not use this agent for doc-comment format or mandated presence (the language idiom reviewers own official format), commented-out code blocks (simplicity-reviewer owns dead code), or style, naming, architecture, or performance concerns — this agent evaluates only whether each comment carries information the code cannot, and whether that information is true. This agent only reports findings and never edits code.\n\n<example>\nContext: The user finished a feature and wants the comments checked before committing.\nuser: "The feature works. Can you check the comments aren't just noise before I commit?"\nassistant: "I'll use the comment-reviewer agent to check the changed files for comments that restate the code, stale claims, and doc comments missing the real contract."\n<commentary>\nPre-commit review with an explicit comment-quality concern triggers the comment-reviewer agent, scoped to the recently changed code (git diff).\n</commentary>\n</example>\n\n<example>\nContext: The user suspects a codebase is full of obvious comments.\nuser: "This package comments every other line — '// increment i' style. Can you audit which comments are worth keeping?"\nassistant: "Let me launch the comment-reviewer agent to classify the package's comments: restating noise to delete, journal prose that belongs in git, and the few why-comments worth keeping."\n<commentary>\nThe user explicitly questions comment value, which is exactly what the comment-reviewer agent evaluates: content the code cannot express versus noise.\n</commentary>\n</example>\n\n<example>\nContext: The user just wrote doc comments for a public API.\nuser: "I've documented the public functions in pkg/store. Does the documentation actually help callers?"\nassistant: "I'll review pkg/store with the comment-reviewer agent to verify each doc comment states the contract — errors, boundaries, ownership — instead of echoing the signature."\n<commentary>\nNewly written documentation is a prime candidate for an echo-doc and accuracy check before it ossifies.\n</commentary>\n</example>\n\n<example>\nContext: The assistant has just implemented a module containing a workaround with several comments.\nuser: "Add retry logic around the flaky upstream call."\nassistant: "Here is the implementation. Since it includes a workaround with explanatory comments, I'll proactively run the comment-reviewer agent to verify each comment earns its keep and the workaround rationale is properly recorded."\n<commentary>\nThe assistant just produced comment-bearing code, so it proactively invokes the comment-reviewer agent without waiting for the user to ask.\n</commentary>\n</example>
model: inherit
color: blue
