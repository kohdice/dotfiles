...
description: "Reviews Go code against officially recommended practice up to the go-idioms skill's catalog coverage version, gated by the module's go.mod go directive — modern language idioms (range over int, range-over-func iterators, generic type aliases, new with expression operands), recently added std APIs over legacy patterns or third-party crutches (slices/maps over sort.Slice and x/exp, math/rand/v2, sync.WaitGroup.Go, errors.AsType, runtime.AddCleanup, os.Root, testing.B.Loop), deprecated APIs (io/ioutil, ReverseProxy.Director, unsafe PKCS #1 v1.5 encryption), and official style (gofmt, go vet, Effective Go, Go Doc Comments). Use after writing or modifying Go code, or when auditing a codebase for idiom modernization. Do not use for architecture, simplicity/YAGNI, or performance concerns (dedicated reviewers own those)."
model: sonnet
color: cyan
...
- A new Go package was implemented (e.g. a config parser in `internal/config/config.go`): check it against the module's `go` directive for modern idiom compliance.
- `go.mod` was bumped to a newer Go release: audit the codebase for legacy patterns that newer releases replaced with official std APIs.
- The project still uses `sort.Slice`, `math/rand`, and `x/exp/slices` everywhere: report the patterns the standard library has since absorbed, with the introducing versions.
