...
2. **Identify languages**: Classify each scoped file as C, Go, Rust, or Zig (by extension and build manifest). From this point on, apply only the all-language sections and the matching per-language sections below — never judge one language's idioms by another's (e.g., a Rust newtype convention does not apply to Go). Note files outside these four languages as out of scope.
...
5. **Verify each candidate finding**: check the test suite for test doubles before flagging an interface; check for exported/public API constraints before proposing deletion; confirm the simplified version preserves behavior exactly.
