...
description: "Reviews C, Go, Rust, or Zig code for implementation that does not earn its keep — pass-through wrappers, single-implementation interfaces/traits with no test seam, premature generics, unused parameters and options, dead code, and layers built for an imagined future with a single caller — per YAGNI and Kent Beck's Tidy First? tidyings. Use after writing or modifying code, before committing, when auditing a codebase for over-engineering, or proactively right after producing a new abstraction layer. Reports findings only; never edits code (to apply simplifications, use pr-review-toolkit:code-simplifier or the /simplify skill). Do not use for style, naming, architecture, or performance concerns (dedicated reviewers own those)."
model: sonnet
color: yellow
...
- A feature is done and the author wants an over-engineering check before committing: needless wrappers, speculative abstractions, and YAGNI violations in the changed files.
- A Go service package feels like "all indirection" — interfaces everywhere with one implementation: audit for single-implementation interfaces, pass-through methods, and layers with a single caller.
- A Rust config module was just written with a builder and a `ConfigSource` trait: verify the builder and trait earn their complexity, or whether a plain struct would do.
- The assistant itself just introduced an interface with a single implementation (e.g. a storage layer): run this reviewer proactively before the abstraction ossifies.
